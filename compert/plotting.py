...
class CompertVisuals:
    ...
    def plot_latent_embeddings(
        ...
        if filename is None:
            if self.fileprefix is None:
                ...
                file_name_similarity = (
                    f"{self.fileprefix}_emebdding_similarity.png"
                )
        else:
        ...
        if len(emb) < 2:
            print(
                f"Embedding contains only {len(emb)} vectors. Not enough to plot."
            )
        else:
            plot_embedding(
    ...
    def plot_contvar_response(
        ...
        if logdose:
            ...
            df_response[dose_name] = log10_with0(
                df_response[self.dose_key].values
            )
            if not (df_ref is None):
...
def plot_uncertainty_comb_dose(
    ...
    Z = np.array(df_pred[f"uncertainty_{metric}"].values.astype(float)).reshape(
        N, N
    )

    fig, ax = plt.subplots(1, 1)
    CS = ax.contourf(
        X, Y, Z, cmap="coolwarm", levels=20, alpha=1, vmin=vmin, vmax=vmax
    )
...
def plot_uncertainty_dose(
    ...
    if log:
        if min_dose is None:
            min_dose = 1e-3
        N_val = np.round(
            np.logspace(np.log10(min_dose), np.log10(1), N), decimals=10
        )
    else:
        if min_dose is None:
            min_dose = 0
...
def plot_r2_matrix(pred, adata, de_genes=None, **kwds):
    ...
    for cond in conditions:
        ...
        for cond_real in conditions:
            ...
            r2s_var[cond_real].append(
                r2_score(y_true.var(axis=0), y_pred.var(axis=0))
            )
...
class ComPertHistory:
    ...
    def __init__(self, history, fileprefix=None):
        ...
        for split in ["training", "test", "ood"]:
            df_split = pd.DataFrame(
                np.array(history[split]), columns=self.header
            )
    ...
    def plot_losses(self, filename=None):
        ...
        fig, ax = plt.subplots(
            1, 4, sharex=True, sharey=False, figsize=(12, 2.0)
        )
    ...
    def plot_metrics(self, epoch_min=0, filename=None):
        ...
        fig, axs = plt.subplots(
            3, 2, sharex=True, sharey=False, figsize=(7, 7.0)
        )
        ax = plt.gca()
        i = 0
        for i1 in range(2):
