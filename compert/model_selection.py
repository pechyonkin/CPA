...
def get_best_plots(model_name, path="./results/plots"):
    ...
    plots_prefix = (
        f"{folder}/{model_name.split('/')[-2]}_{model_name.split('/')[-1]}"
    )
    ...
    df_ref["log10-dose"] = [
        np.log10(float(d)) for d in df_ref[selected_drug].values
    ]

    df_uncert["log10-dose"] = [
        np.log10(float(d)) for d in df_uncert["dose_val"].values
    ]
