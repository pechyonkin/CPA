...
class ComPertAPI:
    ...
    def __init__(self, datasets, model):
        ...
        for k in datasets.keys():
            ...
            for pert in np.unique(datasets[k].pert_categories):
                num_points = len(
                    np.where(datasets[k].pert_categories == pert)[0]
                )
        ...
        self.measured_points["all"] = copy.deepcopy(
            self.measured_points["training"]
        )
    ...
    def get_covars_embeddings(self, return_anndata=True):
        ...
        """
        self.emb_covars = (
            self.model.cell_type_embeddings(
    ...
    def mix_drugs(self, drugs_list, doses_list=None, return_anndata=True):
        ...
        for i, drug_combo in enumerate(drugs_list):
            drug_mix[i] = self.get_drug_encoding_(
                drug_combo, doses=doses_list[i]
            )
    ...
    def latent_dose_response(
        ...
        for drug in perturbations:
            ...
            if self.model.doser_type == "mlp":
                response = (
                    (
                        self.model.dosers[d](this_drug).sigmoid()
                        * this_drug.gt(0)
                    )
                    .cpu()
                    .clone()
    ...
    def latent_dose_response2D(
        ...
        assert (
            len(perturbations) == 2
        ), "You should provide a list of 2 perturbations."
            ...
            if self.num_measured_points["training"][cov_pert] > thrh:
                ...
                if "+" in pert_loop:
                    ...
                    for _dose in pd.Series(dose_loop_list).unique():
                        ...
                        tmp_ad.obs["pert_dose"] = (
                            tmp_ad.obs.condition + "_" + _dose
                        )
                        emb_perts_loop.append(tmp_ad)

                    emb_perts_loop = emb_perts_loop[0].concatenate(
                        emb_perts_loop[1:]
                    )
                    X = emb_covars.X[
                        emb_covars.obs.cell_type == cov_loop
                    ] + np.expand_dims(
                        emb_perts_loop.X[
                ...
                tmp_ad.obs["cov_pert"] = "_".join(
                    [cov_loop, pert_loop, dose_loop]
                )
    ...
    def compute_uncertainty(self, cov, pert, dose, thrh=30):
        ...
        covar_ohe = torch.Tensor(self.covars_dict[cov]).to(self.model.device)

        drug_ohe = torch.Tensor(self.get_drug_encoding_(pert, doses=dose)).to(
            self.model.device
        )
        ...
        drug_emb = (
            self.model.compute_drug_embeddings_(pert).detach().cpu().numpy()
        )
        cell_emb = (
            self.model.cell_type_embeddings(cov.argmax(1))
            .detach()
            .cpu()
            .numpy()
        )
    ...
    def predict(
        ...
        for i in range(len(df)):
            ...
            covar_ohe = torch.Tensor(self.covars_dict[covar_name]).to(
                self.model.device
            )

            drug_ohe = torch.Tensor(
                self.get_drug_encoding_(comb_name, doses=dose_name)
            ...
            gene_reconstructions = (
                self.model.predict(genes, drugs, covars)
                .cpu()
                .clone()
                .detach()
                .numpy()
    ...
    def get_response(
        ...
        for ict, ct in enumerate(self.unique_сovars):
            ...
            genes_control = (
                datasets[control_name]
                .genes[datasets[control_name].cell_types_names == ct]
                .clone()
                .detach()
            )
            if len(genes_control) < 1:
                print(
                    "Warning! Not enought control cells for this covariate.\
            ...
            if ncells_max < len(genes_control):
                ...
                idx = torch.LongTensor(
                    np.random.choice(
                        range(len(genes_control)), ncells_max, replace=False
                    )
                )
                ...
                if not (drug in datasets[control_name].ctrl_name):
                    for dose in doses:
                        ...
                        gene_means, _, _ = self.predict(
                            genes_control.cpu().detach().numpy(),
                            df,
    ...
    def get_response_reference(self, datasets, perturbations=None):
        ...
        for split in ["training_treated", "ood"]:
            ...
            for pert in self.seen_covars_perts[split]:
                ...
                if drug in perturbations:
                    ...
                    genes_control = (
                        dataset_ctr.genes[(dataset_ctr.cell_types_names == ct)]
                        .clone()
                        .detach()
                    )
                    if len(genes_control) < 1:
    ...
    def get_response2D(
        ...
        genes_control = (
            ...
            .genes[datasets["test_control"].cell_types_names == covar]
            .clone()
            .detach()
        )
        if len(genes_control) < 1:
        ...
        ncells_max = min(ncells_max, len(genes_control))
        idx = torch.LongTensor(
            np.random.choice(range(len(genes_control)), ncells_max)
        )
        genes_control = genes_control[idx]
        ...
        dose_vals = [
            f"{d[0]}+{d[1]}" for d in itertools.product(*[doses, doses])
        ]
    ...
    def get_cycle_uncertainty(
        ...
        if direction == "forward":
            ...
            return np.mean((genes_control - gene_return) ** 2), 1 - r2_score(
                ctr, ret
            )
        else:
            ...
            return np.mean(
                (gene_condition - gene_condition_return) ** 2
            ), 1 - r2_score(ctr, ret)
    ...
    def print_complete_cycle_uncertainty(
        ...
        ctr_covar, ctrl_name, ctr_dose = datasets_ctr.pert_categories[0].split(
            "_"
        )
    ...
    def evaluate_r2(self, dataset, genes_control):
        ...
        for pert_category in np.unique(dataset.pert_categories):
            ...
            if len(idx) > 0:
                emb_drugs = (
                    dataset.drugs[idx][0].view(1, -1).repeat(num, 1).clone()
                )
                emb_cts = (
                    dataset.cell_types[idx][0]
                    .view(1, -1)
                    .repeat(num, 1)
                    .clone()
                )
                ...
                genes_predict = (
                    self.model.predict(genes_control, emb_drugs, emb_cts)
                    .detach()
                    .cpu()
                )
...
def get_reference_from_combo(
    perturbations_list, datasets, splits=["training", "ood"]
):
    """
...
def evaluate_r2_benchmark(
    compert_api, datasets, pert_category, pert_category_list
):
    scores = pd.DataFrame(
        columns=[
    ...
    if len(idx) > 0:
        for pert_category_predict in pert_category_list:
            if "+" in pert_category_predict:
                ...
                idx_pred1 = np.where(
                    datasets["training"].pert_categories == pert1
                )[0]
                idx_pred2 = np.where(
                    datasets["training"].pert_categories == pert2
                )[0]
            ...
            #                     yp_v = (y_pred1.var(axis=0) + y_pred2.var(axis=0))/2
                ...
                idx_pred = np.where(
                    datasets["training"].pert_categories
